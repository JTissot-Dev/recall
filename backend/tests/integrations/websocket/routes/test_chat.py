...
def test_websocket_chat_endpoint(MockChatService, mock_db_session, client):
    ...
    with client.websocket_connect("/ws/chat") as websocket:
        ...
        # Assert
        mock_chat_service.init_chat_session.assert_called_once_with(mock_db_session, None)
...
@patch('app.websocket.routes.chat.ChatService')
def test_websocket_chat_endpoint_with_client_id(MockChatService, mock_db_session, client):
    # Arrange
    client_message = "Hello !"
    assistant_response = "Hi, how can i help you today ?"
    ...
    mock_conversation = Conversation(
        ...
        messages=[
            mock_existing_user_message,
            mock_existing_assistant_message
        ],
        created_at=datetime.now(),
        updated_at=datetime.now()
    )
    ...
    # Act
    with client.websocket_connect("/ws/chat?conversation_id=488098a9-4c50-4786-87be-89628cb1a5b9") as websocket:
        ...
        mock_chat_service.init_chat_session.assert_called_once_with(
            mock_db_session, 
            "488098a9-4c50-4786-87be-89628cb1a5b9"
        )
        mock_chat_service.process_chat_session.assert_called_once_with(
            mock_db_session, client_message, mock_conversation
        )
