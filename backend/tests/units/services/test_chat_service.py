...
def test_init_chat_session_with_existing_conversation(
    ...
    # Act
    conversation = chat_service.init_chat_session(
        mock_db_session, str(conversation_id)
    )

    # Assert
    read_conversation_by_id.assert_called_once_with(
        mock_db_session, str(conversation_id)
    )
...
def test_init_chat_session_with_new_conversation(
    ...
    mock_create_conversation = ConversationCreate(title=expected_title)
    mock_db_conversation = Conversation(
    ...
    create_conversation.assert_called_once_with(
        mock_db_session, mock_create_conversation
    )
    assert conversation == mock_db_conversation
...
def test_process_chat_session(
    ...
    mock_message_history = [
        ChatMessage(
            role=mock_user_message.role, content=mock_user_message.content
        ),
        ChatMessage(
    ...
    mock_chat_message = ChatResponse(
        response=assistant_response, updated_history=mock_message_history
    )

    mock_ollama_provider.process_chat_message.return_value = mock_chat_message

    # Act
    response = chat_service.process_chat_session(
        mock_db_session, client_message, mock_db_conversation
    )
